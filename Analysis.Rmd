...
We will now create a Decision Tree to predict the average monthly net salary based on the price of an apartment in the center, the price of an apartment outside of the center, basic utilities, the price of an inexpensive meal, the price of the internet, the price of a meal for two, the price of 1 gallon of gas, the price of a McMeal, and the 1 year tuition for one child at an international primary school.
```{r}
...
Both the linear regression and the decision tree give informative salary predictions, but the decision tree was able to take all of the variables as predictors (even those that do not have a linear relationship with salary) and told us which variables are the most significant predictors in our model. Two of the predictors are the same in the regression model and the decision tree model, but one variable differs. In terms of interpretability, both models are easy to interpret. In our case we would prefer the linear regression model because it gives us specific estimates for our coefficients, which we can use in predicting the average monthly net salary in cities not included in our dataset. 
...
Holding all other variables constant, for 100 dollar increase in the apartment price outside city center, the estimated price for apartment price in city center will increase 106 dollar on average.

Holding all other variables constant, for one dollar increase in the meal for two, the estimated price for apartment price in city center will increase 9.5 dollar on average.
...
xgbc = xgb.train(data = xgb_train, max.depth = 2, nrounds = 50)
...
```
Here, we use the gradient boosting trees algorithm to train the model. From this model we could get the relative importance in prediction of the apartment price in the city center. The gain means how much each predictors contributed to the model. From this machine learning model, it's apt_out_center as the most importance one, the salary as the second importance one. The cover means the relative number of observations, and apt_out_center has the highest cover. Frequency is the percentage measuring the relative time a particular tree, and the result is similar as the previous two measure where apt_out_center has the top percentage.

The importance of the predicted variables is similar as the regression model.
