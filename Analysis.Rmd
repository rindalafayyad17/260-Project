...
combined %>% ggplot() + 
  geom_histogram(aes(x = apt_in_center), binwidth = 200, color = "black", fill = "light blue") + 
...
```
Some variables XXXX seems have linear relationship with aparment price in City center, and some may have quadratic relationship. XXXXX
...
#Seperate into training set and test set with 60% training and 40% test set
set.seed(1)

x <- stratified(combined, "apt_in_center", 0.6, keep.rownames = TRUE)
...
```{r}
train_set %>% summary()
mean(train_set$apt_in_center)
train_set$apt_in_center

fit_naiveBayes = naiveBayes(apt_in_center ~ apt_out_center + basics + salary  +     inexpensive_meal + primary_school + meal_for_2, data = train_set)

p_hat_nb <- predict(fit_naiveBayes, test_set, type = "raw")[,2]
y_hat_naiveBayes = factor(ifelse(p_hat_nb > 0.5, "1", "0"))

print(p_hat_nb)
confusionMatrix(data = y_hat_naiveBayes, 
                reference = as.factor(test_set$apt_in_center), 
                positive = "1")
...
```{r}
set.seed(1)
rf_fit<- randomForest(apt_in_center ~ apt_out_center + basics + salary  +     inexpensive_meal + primary_school + meal_for_2, data = train_set)

p_hat_rf <- predict(rf_fit, newdata = test_set)
```
