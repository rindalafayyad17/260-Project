    ...
    output$comparison <- renderPlot({
        ...
        p1 <- ggplot(offers, aes(x = city, y = expenses, label = expenses)) +
            ...
            labs(title = "Monthly Expenses",
                 x = "$") +
            geom_text(aes(label = paste0("$",round(expenses,2)), y = expenses + 50), size = 4, fontface = "bold") +
            theme_bw() +
            theme(
                axis.title.x = element_blank(),
        ...
        p2 <- offers %>% 
            ...
            geom_text(aes(label = paste0("$",round(month_sal,2)), y = month_sal + 50), size = 4, fontface = "bold") +
            theme_bw() +
            theme(
                axis.title.x = element_blank(),
                axis.ticks.x = element_blank(),
        ...
        p3 <- offers %>%
            ...
            geom_text(aes(label = paste0("$",round(monthly_remainder,2)), y = monthly_remainder + 50), size = 4, fontface = "bold") +
            theme_bw() +
            theme(
                axis.title.x = element_blank(),
                axis.title.y = element_blank(),
